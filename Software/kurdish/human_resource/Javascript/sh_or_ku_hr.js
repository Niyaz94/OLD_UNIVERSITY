				...
				function(data, status){	
					...
					for(var i=0;i<data.length;i++){
						...
						if(data[i]["service_type"]=="annual"){
							data[i]["service_type"]="سه‌رمووچه‌ی ساڵانه‌";
						}else if(data[i]["service_type"]=="thanks"){
							data[i]["service_type"]="پێشخستنی سوپاس و پێزانین";
						}else if(data[i]["service_type"]=="promotion"){
							data[i]["service_type"]="به‌رزكردنه‌وه‌";
						}else if(data[i]["service_type"]=="certificate"){
							data[i]["service_type"]="پێشخستنی بروانامه‌";
						}else if(data[i]["service_type"]=="service_calculation"){
							data[i]["service_type"]="هه‌ژماری راژه‌";
						}
						for(var j=0;j<2;j++){
