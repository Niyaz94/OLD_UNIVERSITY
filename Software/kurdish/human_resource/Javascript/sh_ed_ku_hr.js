$(document).ready(function(){
  ...
  $("#promotion_table tbody").on("click","button",function(){
		...
		if(id>0){//ئه‌گه‌ر هیچ ناوه‌ك دیاری نه‌كرابوو ئه‌وه‌ هیچ مه‌كه‌
			...
			{"id":id,"type":"type"},
				...
				if (confirm('دڵنیای ده‌توێ ناوی ئه‌و فه‌رمانبه‌ره‌ بسریته‌وه‌')) {
					$.post("../PHP/sh_ed_ku_proc_hr.php",
					{"id":id,"remove":"remove"},
						function(data, status){
							if(data==1)
  ...
  $("#save_information").click(function(){	
		...
		if(id>0){
			$.post("../PHP/sh_ed_ku_proc_hr.php",{"id":id,employee: emp_json, department: dep_json, information:info_json},
				function(data, status){
					$("#myModal").modal("hide");			
				}
			);
