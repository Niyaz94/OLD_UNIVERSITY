$(document).ready(function(){
	...
	$("#emp_id").click(function(){
		...
		$("#change_text").text("بەش");
	});
	
	$("#dep_form_pre").click(function(){
		$("#department_form").hide();
		$("#employee_form").show();
		$("#change_text").text("فەرمانبەر");
	});
	...
	$("#cer_form_save").click(function(){	
		...
		cer_json=JSON.stringify(cer_json);
			...
			$.post("../PHP/empl_ku_proc_hr.php",{employee: emp_json, department: dep_json, information:info_json, certificate:cer_json},
	...
	$('#name,#username').blur(function(){	
		...
		if(check_id=="name"){
			...
			if($(this).val().length>0){
			$.post("../PHP/empl_ku_proc_hr.php",{"check":"check","username":$(this).val()},
				function(data, status){
					if(data==0){
						alert("ئه‌و ناوه‌ دووباره‌یه‌\n تكایه‌ ناوێكی تر داخیل بكه‌");
