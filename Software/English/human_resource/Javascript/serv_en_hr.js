			...
			{"info":value,"order":order,"id":id},
				function(data, status){
					if(data==1){
						...
						$("#td1").text(info_value[2]);
						$("#td2").text(parsed.title);
						$("#td3").text(info_value[0]);
						$("#td4").text(parsed.salary);
						$("#td5").text(info_value[3]);
						$("#td6").text(parsed.rank);
						$("#td7").text(info_value[4]);
						$("#td8").text(parsed.grade);
						$("#td9").text(info_value[1]);
						$("#td10").text(parsed.S_date);		
						close();
						$("#myModal2").modal("hide");
					}
