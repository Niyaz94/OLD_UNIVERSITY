$(document).ready(function(){
				...
				function(data, status){	
					...
					for(var i=0;i<information.length;i++){
						...
						if(information[i]=="barzkrdnaua"){
								...
								if(data[0]["molat"]==1){
									$("#"+information[i]).val("Have Leave");
								}else if(data[0]["molat"]==0){
									$("#"+information[i]).val("No Leave");
								}
  ...
  $("#save_information").click(function(){	
		...
		if(id>0){
			$.post("../PHP/sh_ed_en_proc_hr.php",{"id":id,employee: emp_json, department: dep_json, information:info_json},
				function(data, status){
					$("#myModal").modal("hide");			
				}
			);
